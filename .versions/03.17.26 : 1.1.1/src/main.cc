            ...
            for (const auto& entry : fs::directory_iterator(path)) {
                ...
                if (!Utilities::explore_all) { // If not a full scan (i.e., shallow scan)
                    ...
                    if (entry.is_directory()) {
                        // If it's a directory [DIR] is added to the end of the output
                        snprintf(buffer, sizeof(buffer), "Found : %s [DIR]", entry.path().string().c_str());
                        // For recursive calls, pass the current current's path directly.
                        ...
                        snprintf(gitignore_buffer, sizeof(gitignore_buffer), "%s", entry.path().filename().string().c_str());
                        if (strcmp(gitignore_buffer, ".gitignore") == 0) {
                            // printf("%s\n", gitignore_buffer);

                            // Open the .gitignore file and read its contents
                            std::ifstream gitignore_file(entry.path());
                            if (gitignore_file.is_open()) {
                                ...
                                while (std::getline(gitignore_file, line)) {
                                    ...
                                    snprintf(ignore_buffer, sizeof(ignore_buffer), "%s", line.c_str());
                                    // printf("%s\n", ignore_buffer);
                                }
                        ...
                        // If it's a file nothing is added and output stays the same
                        if (gitignore_buffer[0] != '\0') {
                            if (strcmp(ignore_buffer, entry.path().string().c_str()) == 0) {
                                continue;
                            }
                            snprintf(buffer, sizeof(buffer), "Found : %s", entry.path().string().c_str());
                        }
